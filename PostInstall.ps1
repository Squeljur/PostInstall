...
if (Test-Path "$ProgramX86\Microsoft\Edge\Application")
{
    ...
    New-ItemProperty -Path "HKLM:\So# Make sure to have admin permissions
    param([switch]$Elevated)
    ...
    ftware\Microsoft\Windows\CurrentVersion\Explorer" -Name DisableEdgeDesktopShortcutCreation -PropertyType DWORD -Value 1
    Write-Host "Microsoft Edge " -NoNewline -ForegroundColor Cyan
    Write-Host "has been removed"
}
else
